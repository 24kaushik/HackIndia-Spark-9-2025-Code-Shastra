...
const Login = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    const response = await fetch("http://localhost:3000/api/v0/auth/user/login", {
      method: "POST",
      headers: {
    ...
    <div className="h-screen w-full flex items-center justify-center bg-gradient-to-br from-gray-900 to-black p-4">
      ...
      >
        ...
        >
          ...
          >
            {[
              { name: "google", icon: <FaGoogle className="w-5 h-5 text-white" /> },
              { name: "github", icon: <FaGithub className="w-5 h-5 text-white" /> },
              { name: "twitter", icon: <FaTwitter className="w-5 h-5 text-white" /> }
            ].map((provider) => (
              <motion.button
                key={provider.name}
